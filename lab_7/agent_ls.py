...
def swap_and_words(input_list,check_both_noun=False):
    ...
    for idx,(_,_,sentence,_) in enumerate(input_list):
        ...
        and_index = next((i for i, (token, pos) in enumerate(tagged_tokens) if token.lower() == 'and'), None)
...
def generate_rhyming_lines(synonyms_that_rhyme, lines, ll1, ll2):
    if synonyms_that_rhyme:
        new_tokens = nltk.word_tokenize(ll2[2])[:-1] + [random.choice(synonyms_that_rhyme)]
...
def add_sensible_adjective(lines):
    ...
    chosen_noun_position,chosen_noun,tokenized = noun_selector(lines[idx_line][2])
